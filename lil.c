...
// oddstream
_Bool lil_empty_value(lil_value_t val)
{
    return val == NULL || val->l == 0;
}
...
lil_value_t lil_eval_expr(lil_t lil, lil_value_t code)
{
    ...
    if (ee.error) {
        ...
        case EERR_SYNTAX_ERROR:
            ...
            break;
        default:    // oddstream
            lil_set_error(lil, "unknown error");
            break;
        }
        return NULL;
    }
...
// oddstream parse a void* out of the stored string
void* lil_to_userdata(lil_value_t val)
{
	void* result = NULL;
	if (val && val->l) {
		sscanf(val->d, "%p", &result);
	}
	return result;
}
...
{
    char buff[320]; // oddstream was 128
    snprintf(buff, 320, "%f", num);   // oddstream, was sprintf
...
// oddstream put the pointer in a string and alloc that
lil_value_t lil_alloc_userdata(void* userdata)
{
	char buff[64];
	snprintf(buff, 64, "%p", userdata);
	return alloc_value(buff);
}
...
static LILCALLBACK lil_value_t fnc_reflect(lil_t lil, size_t argc, lil_value_t* argv)
{
    ...
    if (!strcmp(type, "dollar-prefix")) {
        lil_value_t r2;  // oddstream, r shadows a previous local
        if (argc == 1) return lil_alloc_string(lil->dollarprefix);
        r2 = lil_alloc_string(lil->dollarprefix);
        free(lil->dollarprefix);
